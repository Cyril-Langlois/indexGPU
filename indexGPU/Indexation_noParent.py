...
class IndexationGPUderiv:
    ...
    def __init__(self, Image, savePath, database, CIFfile, Workflow=[['Diff',0]], nScore = 1, normType = "centered euclidian", nbSTACK=20_000, nbDB = 20_000, dimPROF = 180):
    ...
    def dataPrepDiff(self):
        ...
        for i in range(len(self.listChunkArrays)):
            ...
            #  POUR FAIRE UNE FFT DES PROFILS DE LA BASE 
            # self.c = fct.FFT_filtering_prf_InLines(self.c, freq = 1)
            # self.c = fct.centeredEuclidianNorm(self.c, ax = 1) # Renormalisation
            #  POUR FAIRE UNE FFT DES PROFILS DE LA BASE 
    ...
    def disOfinal(self):
        ...
        def disOscore(nOri, SymQ):

            refO = Quaternion(nOri[-1, :]).inverse
            nScoreDisOpix = np.zeros(len(nOri))
            
            for k in range(len(nOri)):
                ori = Quaternion(nOri[k, :]).inverse
                
                nScoreDisOpix[k] = xa.disOfromQuatSymNoMat(refO, ori, SymQ)[1]
            
            return nScoreDisOpix
        ...
        if self.nScore != 1:
            try:
                ...
                self.multiProcDisO = Parallel(n_jobs=20, verbose=1)(delayed(disOscore)(self.nScoresOri[:, :, i, j], self.SymQ) for i in range(self.height) for j in range(self.width))
        
                a = np.asarray(self.multiProcDisO)
                b = np.copy(a.reshape((self.height,  self.width, self.nScore)))
                c = np.swapaxes(b, 0, 2)
                # self.multiProcRes = np.swapaxes(c, 1, 2)
                self.nScoresDisO = np.swapaxes(c, 1, 2)
        
                # print(self.multiProcRes.shape)
                t2 = time.time()
                # print("results : ", results[:10])
                print("multiprocessor disO : ", t2-t1)
            except:
                print("old fashion disO")
            
                # old fashion disO calculation without multiprocessing
                t1 = time.time()
                for i in range(self.height):
                    
                    if i % np.round(0.1*self.height) == 0:
                        print('DisO is running :', f"{(i/self.height):.0%}")
                ...
                t2 = time.time()
                print("usual disO : ", t2-t1)
            
            self.meanDisO = np.mean(self.nScoresDisO, axis = 0)
            
            print('DisO is running :', f"{1:.0%}")
            print(self.nScoresDisO.shape)
            print("end disO")
        else :
            self.meanDisO = np.mean(self.nScoresDisO, axis = 0)
            print("No disO because only one score")
